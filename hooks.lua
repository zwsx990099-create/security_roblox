...
function HLL.new(precision)
    ...
    return setmetatable({
        precision = precision, size = size, buckets = {},
...
function CUSUM.new(threshold, drift)
    return setmetatable({
        threshold = threshold or 5.0, drift = drift or 0.5,
        sumPos = 0, sumNeg = 0, mean = 0, samples = 0,
        lastAlert = 0, alertCount = 0,
    }, CUSUM)
end
        ...
        if event == "call" then
            ...
            if info then
                ...
                if shouldSample("call") then
                    emit("OPCODE_CALL", {
                        name = info.name, source = info.short_src,
                        line = info.currentline, depth = State.stackDepth,
                    }, 0)
                end
            end
        end
        ...
        __newindex = function(t, k, v)
            ...
            if allowRate("global_write:" .. tostring(k)) then
                ...
                if shouldSample("global_write") then
                    emit("GLOBAL_WRITE", {
                        key = tostring(k),
                        old_type = type(old), new_type = type(v),
    ...
    coroutine.resume = function(co, ...)
        ...
        if allowRate("co_resume") then
            emit("COROUTINE_RESUME", {
                ...
                total = resumeCounter, dt = dt, filteredDt = filtered,
            }, 0)
        end
    ...
    if type(env.HttpGet) == "function" then
        ...
        local wrapped = function(url, ...)
            ...
            emit("HTTP_GET", {
                url = tostring(url):sub(1, 500),
                suspicious = suspicious, tag = tag,
                source = "env.HttpGet", stack = captureStack(5),
            }, suspicious and 3 or 1)
            return orig(url, ...)
        end
        if newcclosure then pcall(function() wrapped = newcclosure(wrapped) end) end
    ...
    if type(env.request) == "function" then
        ...
        local wrapped = function(opts)
            ...
            emit("NETWORK_REQUEST", {
                url = tostring(url):sub(1, 500), method = method,
                suspicious = suspicious, tag = tag,
                source = "env.request", stack = captureStack(5),
            }, suspicious and 3 or 1)
            return orig(opts)
        end
        if newcclosure then pcall(function() wrapped = newcclosure(wrapped) end) end
        ...
        if type(orig) == "function" then
            ...
            local wrapped = function(path, ...)
                ...
                emit(entry.eventType, {
                    ...
                    sensitive = sensitive, func = entry.name,
                    stack = captureStack(5),
                }, sev)
                return orig(path, ...)
            end
            ...
            then
                if allowRate("string_char") and shouldSample("string_char") then
                    ...
                    emit("STRING_DECRYPT", {
                        ...
                        length = #result, entropy = ent,
                        b64 = looksBase64(result), hex = looksHex(result),
                        suspicious = suspicious, tag = tag,
                        source = "string.char",
                    }, suspicious and 3 or 1)
                end
            ...
            if t - lastFlush >= 1 then
                ...
                if signal == "up" and counter >= 100 then
                    if allowRate("bxor_burst") then
                        emit("STRING_DECRYPT", {
                            count = counter, source = "bit32.bxor",
                            cusum = dev, stack = captureStack(4),
                        }, 1)
                    end
                end
                counter = 0
        ...
        if type(t) == "table" and type(mt) == "table" then
            ...
            if allowRate("setmetatable") and shouldSample("setmetatable") then
                emit("METATABLE_ACCESS", {
                    op = "set", keys = table.concat(keys, ","):sub(1, 100),
                    stack = captureStack(4),
                }, 1)
            end
        end
            ...
            if info and info.func then
                ...
                while true do
                    ...
                    if type(val) == "string" and #val > 20 and rememberString(val) then
                        ...
                        if suspicious then
                            emit("STRING_DECRYPT", {
                                value = val:sub(1, 200), suspicious = true, tag = tag,
...
local function startSelfIntegrity()
    ...
    for _, w in ipairs(watched) do
        State.baselineHashes[w.name] = tostring(w.fn)
    end

    State.integrityThread = task.spawn(function()
        while State.installed do
            task.wait(Hooks.Config.INTEGRITY_INTERVAL)
            for _, w in ipairs(watched) do
...
function Hooks.install(edr)
    ...
    Hooks._log(1, "install", "hooks installed", { count = #unhooks })
    return true, #unhooks
end
