...
function RingBuffer:push(item)
    ...
    if idx == 0 then idx = self.size end
    if self.count >= self.size then
        self.head = self.head + 1
        if self.head > self.size then self.head = 1 end
...
function RingBuffer:clear()
    self.data = {}
    self.head, self.tail, self.count = 1, 0, 0
...
function BayesianRisk.new(opts)
    ...
    return setmetatable({
        ...
        signals = {},
        signalTimestamps = {},
        signalTTL = opts.signalTTL or 30,
        history = {},
...
function BayesianRisk:addSignal(name, active)
    if active then
        ...
        self.signalTimestamps[name] = clock()
    else
        self.signals[name] = nil
        self.signalTimestamps[name] = nil
    end
end

function BayesianRisk:update()
    local t = clock()
    for sig, timestamp in pairs(self.signalTimestamps) do
        if t - timestamp > self.signalTTL then
            self.signals[sig] = nil
            self.signalTimestamps[sig] = nil
        end
    end
    ...
    table.insert(self.history, {
        t = clock(), posterior = newPosterior, signals = signalCount,
    })
...
function Session:advancePhase(newPhase)
    ...
    self.phase = newPhase
    self.phase_changed = clock()
    return old, newPhase
end

function Session:elapsed() return clock() - self.start_time end
...
function EDR.new()
    ...
    self.watchdog = false
    self.counters = {}
    self.baselineStart = clock()
    self.baselineActive = true
    self.emitRateThisSec = 0
    self.lastEmitReset = clock()
    self.emitDropped = 0
...
function EDR:_applyMode(mode)
    self.mode = mode
    self.timeseries.enabled = EDR.Config.ENABLE_TIMESERIES
    if EDR.Config.ENABLE_MARKOV then
        if not self.markov then self.markov = MarkovDetector.new() end
    else
        self.markov = nil
    end
    if EDR.Config.ENABLE_TAINT then
        if not self.taint then self.taint = TaintTracker.new() end
    else
        self.taint = nil
    end
    if EDR.Config.ENABLE_ENTROPY then
        if not self.entropy then self.entropy = EntropyTracker.new() end
    else
        self.entropy = nil
    end
...
function EDR:emit(eventType, data, severity)
    ...
    local totalKey = "__total__:" .. tFloor
    self.counters[totalKey] = (self.counters[totalKey] or 0) + 1
    ...
    -- Markov: evaluate the transition before recording it so a first-time
    -- transition is not counted as evidence for itself.
    if self.markov then
    ...
    if self.taint then
        ...
        if taintFlow and self.risk and meta then
...
function EDR:_cleanupCounters()
    local nowFloor = math.floor(clock())
    local checked = 0
...
function EDR:getRate(eventType, window)
    ...
    local nowFloor = math.floor(clock())
    local total = 0
    local prefix = eventType == "*" and "__total__:" or (eventType .. ":")
    for i = 0, window - 1 do
        total = total + (self.counters[prefix .. (nowFloor - i)] or 0)
    end
    return total / window
end
    ...
    task.spawn(function()
        ...
        while self.watchdog do
            ...
            if self.baselineActive then
                if (clock() - self.baselineStart) >= EDR.Config.BASELINE_DURATION then
            ...
            if self.timeseries.enabled then
                self.timeseries:push("event_rate", self:getRate("*", 5), clock())
            end
