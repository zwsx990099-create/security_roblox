...
Main.MODULE_REGISTRY = {
    {
        key = "EDR", file = "edr_core.lua",
        required = true, version = "2.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = {}, priority = 100,
        description = "Event bus + detection kernel",
    },
    {
        key = "Hooks", file = "hooks.lua",
        required = true, version = "2.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = { "EDR" }, priority = 90,
        description = "Behavior hooks",
    },
    {
        key = "Rules", file = "rules.lua",
        required = true, version = "2.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = { "EDR" }, priority = 80,
        description = "Rule engine",
    },
    {
        key = "Report", file = "report.lua",
        required = true, version = "2.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = { "EDR", "Rules" }, priority = 70,
        description = "Report generator",
    },
    {
        key = "RobloxAPI", file = "roblox_api.lua",
        required = false, version = "2.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = { "EDR" }, priority = 60,
        description = "Roblox API monitor",
    },
    {
        key = "Vuln", file = "vuln_scanner.lua",
        required = false, version = "2.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = { "EDR" }, priority = 55,
        description = "Vulnerability scanner",
    },
    {
        key = "UI", file = "ui.lua",
        required = false, version = "3.0.0", min_version = "2.0.0",
        hash = "TOFU", deps = { "EDR", "Rules", "Report" }, priority = 50,
...
local function sha256_pure(s)
    -- ใช้ bit32 (Roblox มีให้)
    local band, bxor, bor, rrot = bit32.band, bit32.bxor, bit32.bor, bit32.rrotate
    ...
    for chunk = 1, #s, 64 do
        ...
        for j = 0, 15 do
            ...
            w[j] = bor(bor(bor(b1 * 0x1000000, b2 * 0x10000), b3 * 0x100), b4)
        end
        for j = 16, 63 do
...
function TrustStore.verify(moduleKey, content)
    ...
    for _, e in ipairs(Main.MODULE_REGISTRY) do
        if e.key == moduleKey then registryEntry = e; break end
    end
    ...
    if registryEntry and registryEntry.hash
        ...
        if actualHash ~= expected then
            return false, "hash mismatch: expected " .. expected:sub(1, 16) .. "... got " .. actualHash:sub(1, 16) .. "..."
        end
    ...
    if Main.Config.TOFU_MODE then
        ...
        if stored ~= actualHash then
            return false, "TOFU hash changed: expected " .. stored:sub(1, 16) .. "... got " .. actualHash:sub(1, 16) .. "..."
        end
        return true, actualHash
    end
...
local function fetchWithFallback(filename, baseUrls)
    ...
    for i, base in ipairs(baseUrls) do
        ...
        local ok, content = pcall(function()
            return game:HttpGet(url, Main.Config.URL_TIMEOUT)
        end)
...
local function applyPerformanceMode()
    ...
    -- Auto-select
    if mode == "auto" then
        if tier == "low" then mode = "light"
        elseif tier == "high" then mode = "balanced"  -- ยังไม่ paranoid โดยอัตโนมัติ
    ...
    if mode == "light" then
        ...
        -- ส่งต่อไป hooks ด้วย
        if Main.State.modules.Hooks and Main.State.modules.Hooks.Config then
            Main.State.modules.Hooks.Config.OPCODE_HOOK_COUNT = 3000
            Main.State.modules.Hooks.Config.BASE_SAMPLE_RATE = 0.05
            Main.State.modules.Hooks.Config.HIGH_RISK_SAMPLE_RATE = 0.5
        end
    elseif mode == "paranoid" then
        ...
        if Main.State.modules.Hooks and Main.State.modules.Hooks.Config then
            Main.State.modules.Hooks.Config.OPCODE_HOOK_COUNT = 300
            Main.State.modules.Hooks.Config.BASE_SAMPLE_RATE = 0.3
            Main.State.modules.Hooks.Config.HIGH_RISK_SAMPLE_RATE = 1.0
        end
    else  -- balanced
...
function Health.init(name)
    Main.State.module_health[name] = {
        status = "unknown", last_ok = nil, last_err = nil,
        err_count = 0, retry_count = 0, next_retry = nil,
        installed = false, response_time = 0, version = "?",
    }
end
...
function Health.getReport()
    ...
    for name, h in pairs(Main.State.module_health) do
        r[#r + 1] = {
            name = name, status = h.status, err_count = h.err_count,
            retry_count = h.retry_count, installed = h.installed,
            version = h.version,
        }
    end
...
function Circuit.init(name)
    Circuit[name] = {
        state = "closed", fail_count = 0, success_count = 0,
        opened_at = 0, attempts = 0,
    }
end
...
function Circuit.onSuccess(name)
    local c = Circuit[name]; if not c then Circuit.init(name); c = Circuit[name] end
    c.fail_count = 0
    ...
    for _, entry in ipairs(sorted) do
        ...
        for attempt = 1, 3 do
            ...
            local ok, result = pcall(function()
                -- 1. Fetch
                local content, url = fetchWithFallback(entry.file)
                if not content then
                    error("fetch failed: " .. tostring(url))
                end
                ...
                if Main.Config.VERSION_CHECK then
                    ...
                    if modVer then
                        if modVer < (entry.min_version or Main.Config.MIN_MODULE_VERSION) then
                            error(string.format(
...
function Main.installSubsystem(key)
    ...
    local ok, err = pcall(function()
        if module.install then module.install(Main.State.edr) end
    end)
    ...
    if M.Hooks and M.Hooks.isInstalled and Main.State.module_health.Hooks.installed then
        if not M.Hooks.isInstalled() then
            ...
            if Circuit.canCall("Hooks") then
                ...
                if ok then Health.markOK("Hooks"); Circuit.onSuccess("Hooks")
                else Circuit.onFail("Hooks") end
                repairs = repairs + 1
            end
        end
    end
...
function Main.start()
    ...
    local M = Main.State.modules
    if not Main.State.edr then return false end
    ...
    for _, key in ipairs({ "Hooks", "RobloxAPI", "Vuln" }) do
        if M[key] then Main.installSubsystem(key) end
    end
...
function Main.startMonitorLoop()
    ...
    local co = coroutine.create(function()
        while Main.State.running do
            ...
            if Main.Config.ADAPTIVE_INTERVAL and Main.State.edr then
                ...
                if ok and rate then
                    if rate > 20 then interval = Main.Config.MIN_INTERVAL
                    elseif rate < 1 then interval = Main.Config.MAX_INTERVAL end
                end
            end
    ...
    Main.State.monitor_thread = task.spawn(function()
        while Main.State.running do
            ...
            if not ok then Health.markError("MonitorLoop", waitTime); break end
...
function Main.saveReport()
    ...
    if ok then
        ...
        log(1, "report", "save failed", { err = tostring(err) })
    end
end
...
cmd("start", function() Main.start(); return "Started" end)
cmd("stop", function() Main.stop("cmd"); return "Stopped" end)
cmd("pause", function() Main.pause(); return "Paused" end)
cmd("resume", function() Main.resume(); return "Resumed" end)
cmd("kill", function() Main.kill("cmd"); return "Killed" end)
...
cmd("clear", function() Main.State.command_log = {}; return "Cleared" end)
cmd("report", function() Main.saveReport(); return "Saved" end)
cmd("ui", function()
    local d = Main.State.dashboard
    if not d then return "No dashboard" end
    if d.minimized then d:maximize(); return "Shown" else d:minimize(); return "Hidden" end
end)
cmd("min", function()
    local d = Main.State.dashboard
...
function Main.buildGUI()
    ...
    local function update()
        ...
        local st, col
        if Main.State.killed then st, col = "🔴 KILLED", Color3.fromRGB(248, 81, 73)
        elseif Main.State.paused then st, col = "🟡 PAUSED", Color3.fromRGB(210, 153, 34)
        elseif Main.State.running then st, col = "🟢 RUNNING", Color3.fromRGB(126, 231, 135)
        else st, col = "⚪ STOPPED", Color3.fromRGB(200, 90, 90) end
        statusLbl.Text = st
...
function Main.buildDashboard()
    ...
    if not ok or not d then return false, tostring(err or "failed") end
