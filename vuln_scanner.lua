...
if not unpackArgs then
    unpackArgs = function(values, first, last)
        first = first or 1
        last = last or #values
        if first > last then return end
        return values[first], unpackArgs(values, first + 1, last)
    end
end
...
function Sandbox.run(fn, timeout, ...)
    local start = now()
    local args = { ... }
    local co = coroutine.create(function() return fn(unpackArgs(args)) end)
    ...
    for i = 1, maxIter do
        ...
        if dryRun then
            ...
            local ok, result, elapsed = Sandbox.run(function()
                return target(unpackArgs(callArgs))
            end, timeout)
...
local function evidenceKey(evidence)
    ...
    if type(evidence) ~= "table" then
        return tostring(evidence):sub(1, 80)
    end
    local keys = {}
    for key in pairs(evidence) do keys[#keys + 1] = tostring(key) end
    table.sort(keys)
    local parts = {}
    for _, key in ipairs(keys) do
        parts[#parts + 1] = key .. "=" .. tostring(evidence[key])
    end
    return table.concat(parts, "&"):sub(1, 80)
end

local function addFinding(category, severity, mitre, title, detail, evidence, extra)
    local key = category .. ":" .. title .. ":" .. evidenceKey(evidence)
    local t = now()
...
local function scanBufferPatterns()
    ...
    for _, e in ipairs(events) do
        if e.type == "STRING_DECRYPT" and e.data then
            if e.data.source == "string.char" then
                stringChar = stringChar + 1
            elseif e.data.source == "string.gsub" then
