...
function TimeSeries:stats()
    ...
    local sorted = {}
    local start = n < self.capacity and 1 or self.head
    for i = 0, total - 1 do
        local index = start + i
        if index > self.capacity then index = index - self.capacity end
        sorted[i + 1] = self.values[index]
    end
    table.sort(sorted)
    return {
        count = n,
        min = sorted[1] or 0,
        max = sorted[total] or 0,
        avg = self.sum / total,
...
local function untrackInstance(inst)
    ...
    State.trackedCount = State.trackedCount - 1
    for i = #State.trackedOrder, 1, -1 do
        if State.trackedOrder[i] == inst then
            table.remove(State.trackedOrder, i)
            break
        end
    end
end
...
local function startCharacterMonitor()
    ...
    pcall(function()
        local conn = lp.CharacterAdded:Connect(function(char)
            ...
            State.characterBaseline = {
                health = nil, maxHealth = nil, walkSpeed = nil, jumpPower = nil,
            }
...
function RobloxAPI.uninstall()
    ...
    State.propertyBatch = {}
    State.batchSize = 0
    State.batchDropped = 0
    State.serviceAccess = {}
    State.serviceStats = {}
    State.serviceDropped = 0
    State.stats.instanceCreate = 0
    State.stats.instanceDestroy = 0
    State.stats.propertyChanges = 0
    State.stats.servicesAccessed = 0
    State.stats.remotesFound = 0
    State.stats.sampleDropped = 0
    State.stats.sampleKept = 0
    State.remoteCache = setmetatable({}, { __mode = "k" })
    State.remoteCount = 0
    State.lastRemoteScan = 0
    State.cameraBaseline = { fov = nil }
    State.characterBaseline = {
        health = nil, maxHealth = nil, walkSpeed = nil, jumpPower = nil,
    }
