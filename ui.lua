...
local function modeColor(mode)
    if mode == "paranoid" then return UI.Config.DANGER
    elseif mode == "balanced" then return UI.Config.ACCENT
    elseif mode == "light" then return UI.Config.SUCCESS
    else return UI.Config.TEXT_DIM end
end
...
function UI.new(edr, rules, report, main)
    local self = setmetatable({
        ...
        _baselineFunctions = {},
        _stats       = {
            risk = 0, events = 0, alerts = 0,
            vulnCount = 0, health = 100, recovered = 0,
        },
    }, Dashboard)
...
function Dashboard:startIntegrity()
    ...
    for _, w in ipairs(watched) do
        self._baselineFunctions[w.name] = tostring(w.fn)
    end
