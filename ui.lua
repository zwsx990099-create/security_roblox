--[[
    ============================================================
    EDR UI v4.0 — Mobile-First Full Dashboard (Version-Aware)
    ============================================================
    NEW in v4.0:
    - MODULE_VERSION = "4.0.0"
    - setPerformanceMode(mode) — light/balanced/paranoid
    - Mode/Tier display in header
    - Integrity violations in Health tab
    - Structured JSON logging
    - Self-integrity check
    - Mode-aware render throttle
    - Buffer caps for log lines
    - Zero-crash rendering (pcall everywhere)
    - Version + mode + tier in summary
    - getStats() for main.lua

    Tabs (13):
      1.  summary    — session + health + risk + version
      2.  alerts     — live alerts
      3.  vulns      — vulnerabilities + attack surface
      4.  api        — Roblox API stats
      5.  health     — modules + circuit + integrity
      6.  tos        — compliance + audit + antiban
...
function UI.setPerformanceMode(mode)
    ...
    UI.Config.PERFORMANCE_MODE = mode
    UI._log(1, "perf", "mode applied: " .. mode, profile)
    return true
end

function UI.getPerformanceMode()
    return UI.Config.PERFORMANCE_MODE
end
...
function UI._log(level, module, event, data)
    if level > UI.Config.LOG_LEVEL then return end
    if UI.Config.LOG_STRUCTURED then
        local entry = {
            ts = os.time(),
            level = level,
            module = "ui." .. tostring(module),
            event = event,
        }
        if data then entry.data = data end
        print("[EDR] " .. jsonEncode(entry))
    else
        print(string.format("[UI][%s] %s", tostring(module), tostring(event)))
    end
end
...
local function healthColor(score)
    if score >= 90 then
        return UI.Config.SUCCESS
    elseif score >= 70 then
        return UI.Config.CYAN
    elseif score >= 50 then
        return UI.Config.WARN
    elseif score >= 30 then
        return UI.Config.ORANGE
    else
        return UI.Config.DANGER
    end
end
...
local function modeColor(mode)
    ...
    elseif mode == "balanced" then
        return UI.Config.ACCENT
    elseif mode == "light" then
        return UI.Config.SUCCESS
    else
        return UI.Config.TEXT_DIM
    end
end
...
function UI.new(edr, rules, report, main)
    local self = setmetatable({
        ...
        _updateThread        = nil,
        _destroyed           = false,
        _alertAttached       = false,
        _previousOnAlert     = nil,
        _integrityThread     = nil,
    ...
    log(1, "new", "UI instance created",
        { version = UI._VERSION, mode = UI.Config.PERFORMANCE_MODE })
    return self
end
...
function Dashboard:_buildRoot()
    ...
    local bStroke = mk("UIStroke", {
        Color = UI.Config.SUCCESS, Thickness = 2, Parent = bubble,
    })
    mk("UIGradient", {
    ...
    mk("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "🛡  EDR Monitor v" .. UI._VERSION,
    ...
    local modeLbl = mk("TextLabel", {
        Name = "ModeLbl",
        Size = UDim2.new(1, 0, 0, 16),
        Position = UDim2.new(0, 0, 0, 36),
        BackgroundTransparency = 1,
        Text = "mode=balanced | tier=?",
        TextColor3 = UI.Config.TEXT_DIM,
        TextSize = 10,
        Font = Enum.Font.Code,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local cmdBox = mk("TextBox", {
        ...
        BackgroundTransparency = 1,
        Text = "",
        PlaceholderText = "Command (e.g. /help)",
...
function Dashboard:destroy()
    ...
    if self._integrityThread then
        safe(function() task.cancel(self._integrityThread) end)
    end
    if self.gui then
        self.gui:Destroy()
        self.gui = nil
    end
    if self._alertAttached and self.edr
        and self.edr.onAlert == self._alertHandler then
        self.edr.onAlert = self._previousOnAlert
    end
    self._alertAttached = false
end

--========== SELF INTEGRITY ==========--
function Dashboard:startIntegrity()
    if not UI.Config.SELF_INTEGRITY then return end
    if self._integrityThread then return end
    ...
    for _, w in ipairs(watched) do
        self._baselineFunctions[w.name] = tostring((_G and _G[w.name]) or w.fn)
    end

    self._integrityThread = task.spawn(function()
        while not self._destroyed do
            task.wait(UI.Config.INTEGRITY_INTERVAL)
            for _, w in ipairs(watched) do
                local cur = tostring((_G and _G[w.name]) or w.fn)
                if cur ~= self._baselineFunctions[w.name] then
                    self._integrityViolations = self._integrityViolations + 1
                    self._baselineFunctions[w.name] = cur
                end
            end
        end
    end)
...
function Dashboard:_renderSummary()
    ...
    local elapsed = type(s.elapsed) == "function" and s:elapsed() or 0
    self:_addLine("Duration:  " .. fmtDuration(elapsed))
    ...
    if self.main and self.main.State then
        ...
        self:_addLine("Session:   " .. tostring(st.session_id or "?"):sub(1, 16))
    end

    self:_addSpacer(4)
    self:_addHeader("SYSTEM")
    self:_addLine("Version:   v" .. UI._VERSION)
    self:_addLine("Mode:      " .. UI.Config.PERFORMANCE_MODE,
        modeColor(UI.Config.PERFORMANCE_MODE))
    local tier = self.main and self.main.State and self.main.State.device_tier or "?"
    self:_addLine("Tier:      " .. tostring(tier))
    ...
    if self._integrityViolations > 0 then
        self:_addLine("Integrity violations: " .. self._integrityViolations,
            UI.Config.DANGER)
    end
    ...
    if self.report and self.report.risk_curve and self.report.risk_curve.prediction then
        self:_addLine(string.format("Predicted: %.1f%%",
            self.report.risk_curve.prediction * 100), UI.Config.TEXT_DIM, { size = 10 })
    end
...
function Dashboard:_renderVulns()
    ...
    self:_addHeader("VULNERABILITY")
    self:_addLine(string.format("v%s | mode=%s", tostring(sum.version), tostring(sum.mode)),
        UI.Config.TEXT_DIM, { size = 10 })
    self:_addLine(string.format("Total: %d | Crit: %d | High: %d | Med: %d",
        sum.total or 0,
    ...
    if sum.attackSurface then
        ...
        self:_addLine(string.format("  Remotes: %d | Scripts: %d | Sensitive: %d",
            as.remotes_exposed or 0, as.scripts_exposed or 0,
            as.sensitive_paths or 0), UI.Config.TEXT_DIM, { size = 10 })
    end
    ...
    if sum.fuzzing then
        ...
        self:_addLine(string.format("Iter: %d | Hits: %d | Rollback: %d",
    ...
    for i, f in ipairs(top) do
        ...
        local color = SEV_COLOR[sev] or UI.Config.TEXT
        local icon = SEV_ICON[sev] or "·"
        self:_addLine(string.format("%s [%s] %s", icon,
                SEV_LABEL[sev] or "?", tostring(f.title or "?")),
            color, { size = 11, bold = true })
        if f.detail then
...
function Dashboard:_renderApi()
    ...
    self:_addHeader("ROBLOX API MONITOR")
    self:_addLine(string.format("v%s | mode=%s", tostring(s.version), tostring(s.mode)),
        UI.Config.TEXT_DIM, { size = 10 })
    ...
    self:_addLine(string.format("Dropped: %s", fmtNum(s.properties and s.properties.dropped or 0)))
    ...
    if #svcList == 0 then
        ...
        for i, svc in ipairs(svcList) do
            ...
            if svc.count >= 10 then
                color = UI.Config.DANGER
            elseif svc.count >= 3 then
                color = UI.Config.WARN
            end
            self:_addLine(string.format("  %-28s ×%d", tostring(svc.service), svc.count), color)
        end
    end

    if s.integrityViolations and s.integrityViolations > 0 then
        self:_addSpacer(4)
        self:_addLine("Integrity violations: " .. s.integrityViolations,
            UI.Config.DANGER, { bold = true })
    end
...
function Dashboard:_renderHealth()
    ...
    local report = safe(function() return self.main.Health.getReport() end)
    if not report then
        ...
        for _, h in ipairs(report) do
            ...
            local ver = h.version or "?"
            self:_addLine(string.format("%s %-12s v%-6s err=%d",
                    icon, tostring(h.name or "?"), ver, h.err_count or 0),
                color, { size = 11 })
        end
    end
    ...
    self:_addSpacer(4)
    self:_addHeader("SELF-INTEGRITY")
    self:_addLine("UI violations: " .. self._integrityViolations,
        self._integrityViolations > 0 and UI.Config.DANGER or UI.Config.SUCCESS)
    if st and st.integrity_violations then
        self:_addLine("Main violations: " .. st.integrity_violations,
            st.integrity_violations > 0 and UI.Config.DANGER or UI.Config.SUCCESS)
    end
...
function Dashboard:_renderTos()
    ...
    if main.Config and main.Config.ANTIBAN_ENABLED then
        ...
        if main.State.session_start then
            local elapsed = clock() - main.State.session_start
        ...
        self:_addLine(string.format("Max events/s:  %d",
    ...
    self:_addSpacer(4)
    self:_addHeader("POLICY — BLOCKED")
    for _, op in ipairs({
    ...
    self:_addSpacer(2)
    self:_addHeader("POLICY — ALLOWED")
    for _, op in ipairs({
...
function Dashboard:_renderTimeline()
    ...
    for i = start, #entries do
        ...
        if e then
            ...
            if e.data then
                if e.data.url then
                    detail = tostring(e.data.url):sub(1, 35)
                elseif e.data.path then
                    detail = tostring(e.data.path):sub(1, 35)
                elseif e.data.key then
                    detail = tostring(e.data.key)
                elseif e.data.name then
                    detail = tostring(e.data.name)
                elseif e.data.rule then
                    detail = tostring(e.data.rule)
                elseif e.data.value then
                    detail = tostring(e.data.value):sub(1, 35)
                end
            end
...
function Dashboard:_renderIOC()
    ...
    for t, c in pairs(typeCounts) do
        self:_addLine(string.format("  %-16s %d", tostring(t), c),
            UI.Config.TEXT_DIM, { size = 10 })
    end
    self:_addSpacer(4)
    for i = 1, math.min(80, #iocs) do
        local ioc = iocs[i]
        local color = UI.Config.TEXT
        if ioc.risk and ioc.risk >= 0.8 then
            color = UI.Config.DANGER
        elseif ioc.risk and ioc.risk >= 0.5 then
            color = UI.Config.ORANGE
        elseif ioc.type == "url" or ioc.type == "ipv4" then
            color = UI.Config.ORANGE
        elseif ioc.type == "md5" or ioc.type == "sha256" then
            color = UI.Config.PURPLE
        end
...
function Dashboard:_renderRisk()
    ...
    if self.report.risk_curve.prediction then
        self:_addLine("Prediction:    " .. string.format("%.1f%%",
            self.report.risk_curve.prediction * 100), UI.Config.WARN)
    end
    if self.report.forensic_hash then
        self:_addSpacer(4)
        self:_addHeader("FORENSIC HASH")
        self:_addLine(self.report.forensic_hash:sub(1, 48) .. "...",
...
function Dashboard:_renderRules()
    ...
    self:_addLine("แตะปุ่มเพื่อเปิด/ปิด", UI.Config.TEXT_DIM, { size = 10 })
    self:_addSpacer(4)
    for _, r in ipairs(self.rules.ruleOrder) do
        ...
        mk("TextLabel", {
            ...
            BackgroundTransparency = 1,
            Text = string.format("%s [%s] %s", icon,
                SEV_LABEL[r.severity or 0] or "?", r.id or "?"),
            TextColor3 = color,
            TextSize = 11,
            Font = Enum.Font.GothamBold,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        toggleBtn.MouseButton1Click:Connect(function()
            safe(function()
                local rule = self.rules:getRule(ruleId)
...
--========== RENDER: ABOUT ==========--
function Dashboard:_renderAbout()
    self:_addHeader("ABOUT")
    self:_addLine("EDR Lua — Client-Side Behavioral Monitor",
        UI.Config.TEXT_BRIGHT, { bold = true })
    self:_addLine("For educational use only.", UI.Config.TEXT_DIM, { size = 10 })
    self:_addLine("Not an anti-cheat.", UI.Config.DANGER, { size = 10 })
    self:_addLine("", UI.Config.TEXT)

    self:_addHeader("MODULE VERSIONS")
    local modules = {
        { "main.lua",         "4.0.0" },
        { "ui.lua",           UI._VERSION },
        { "edr_core.lua",     "3.0.0" },
        ...
        { "rules.lua",        "3.0.0" },
        { "report.lua",       "3.0.0" },
        { "roblox_api.lua",   "3.0.0" },
        { "vuln_scanner.lua", "3.0.0" },
    }
    for _, m in ipairs(modules) do
        self:_addLine(string.format("  %-18s v%s", m[1], m[2]), UI.Config.TEXT, { size = 10 })
    end

    self:_addSpacer(4)
    self:_addHeader("RUNTIME INFO")
    self:_addLine("UI version:   v" .. UI._VERSION)
    self:_addLine("UI mode:      " .. UI.Config.PERFORMANCE_MODE,
        modeColor(UI.Config.PERFORMANCE_MODE))
    local tier = self.main and self.main.State and self.main.State.device_tier or "?"
    self:_addLine("Device tier:  " .. tostring(tier))
    self:_addLine("Integrity:    " .. self._integrityViolations .. " violations",
        self._integrityViolations > 0 and UI.Config.DANGER or UI.Config.SUCCESS)

    self:_addSpacer(4)
    self:_addHeader("REPOSITORY")
    self:_addLine("github.com/zwsx990099-create/", UI.Config.CYAN, { size = 10 })
    self:_addLine("  security_roblox", UI.Config.CYAN, { size = 10 })

    self:_addSpacer(4)
    self:_addHeader("DISCLAIMER")
    self:_addLine("This tool is a client-side behavioral monitor.",
        UI.Config.TEXT_DIM, { size = 10 })
    self:_addLine("It cannot prevent server-side exploits.",
        UI.Config.TEXT_DIM, { size = 10 })
    self:_addLine("Use of executors violates Roblox ToS.",
        UI.Config.WARN, { size = 10 })
end
...
function Dashboard:update()
    ...
    -- Mode line
    if self.modeLbl then
        local tier = self.main and self.main.State and self.main.State.device_tier or "?"
        self.modeLbl.Text = string.format("mode=%s | tier=%s",
            UI.Config.PERFORMANCE_MODE, tostring(tier))
        self.modeLbl.TextColor3 = modeColor(UI.Config.PERFORMANCE_MODE)
    end
    ...
    if self.riskFill then
        self.riskFill.Size = UDim2.new(clamp(risk, 0, 1), 0, 1, 0)
    ...
    if self.healthFill then
        self.healthFill.Size = UDim2.new(clamp(healthScore / 100, 0, 1), 0, 1, 0)
    ...
    if self.statsLbl then
        self.statsLbl.Text = string.format(
            "Events: %s | Alerts: %d | Vulns: %d | Integrity: %d",
            fmtNum(events), alerts, vulnCount, self._integrityViolations)
    end
...
function Dashboard:attachToEDR()
    if not self.edr or self._alertAttached then return end
    local oldOnAlert = self.edr.onAlert
    local handler = function(alert)
    ...
    self._previousOnAlert = oldOnAlert
    self._alertHandler = handler
    self._alertAttached = true
    self.edr.onAlert = handler
end
...
function Dashboard:getStats()
    return {
        version = UI._VERSION,
        mode = UI.Config.PERFORMANCE_MODE,
        currentTab = self.currentTab,
        minimized = self.minimized,
        destroyed = self._destroyed,
        integrityViolations = self._integrityViolations,
        logLines = #self.logLines,
        cmdHistorySize = #self.cmdHistory,
        stats = {
            risk = self._stats.risk,
            health = self._stats.health,
            events = self._stats.events,
            alerts = self._stats.alerts,
            vulnCount = self._stats.vulnCount,
        },
    }
end
...
UI.MODE_PROFILES = MODE_PROFILES
